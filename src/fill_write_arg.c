...
{
	if (!ft_strcmp(pointer->content, ZJMP_NAME) || !ft_strcmp(pointer->content, LDI_NAME) 
	|| !ft_strcmp(pointer->content, STI_NAME) || !ft_strcmp(pointer->content, FORK_NAME) 
	|| !ft_strcmp(pointer->content, LLDI_NAME) || !ft_strcmp(pointer->content, LFORK_NAME))
		return (0);
	return (1);
}
...
void	fill_write_arg(t_token *pointer, t_strings *rows, t_asm_content **content)
{
	...
	while (pointer)
	{
		if (!ft_strcmp(pointer->type, INSTRACTION_NAME) && check_t_dir_size(pointer))
			...
			write_args(ONE_BYTE, ft_atoiwcm(ft_strsub(pointer->content, 1, ft_strlen(pointer->content))), content);
		else if (!ft_strcmp(pointer->type, DIRECT_NAME))
			write_args(flag ? FOUR_BYTE : TWO_BYTE, ft_atoiwcm(ft_strsub(pointer->content, 1, ft_strlen(pointer->content))), content);
		else if (!ft_strcmp(pointer->type, DIRECT_LABEL_NAME))
			write_args(flag ? FOUR_BYTE : TWO_BYTE, search_instraction(ft_strjoinwcm(ft_strsub(pointer->content, 2, ft_strlen(pointer->content)), ":"), rows) - search_instraction_two(pointer), content);
		else if (!ft_strcmp(pointer->type, INDIRECT_LABEL_NAME))
			write_args(TWO_BYTE, (search_instraction(ft_strjoinwcm(ft_strsub(pointer->content, 1, ft_strlen(pointer->content)), ":"), rows) - search_instraction_two(pointer)), content);

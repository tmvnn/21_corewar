...
char	*label_validation(t_token *pointer, char *content)
{
    ...
    INDIRECT_LABEL) && !ft_strcmp((temp = ft_strsub(pointer->content, 0, strlen\
    (pointer->content) - 1)), (temp1 = ft_strsub(content, 1, strlen(content)))))
	{
		ft_strdel(&temp);
		ft_strdel(&temp1);
		return (content);
...
int flag)
{
	if (!ft_strcmp(pointer->type, INSTRACTION_NAME))
	{
		...
		(*struct_content)->memory_code_size += ONE_BYTE;
	else if (!ft_strcmp(pointer->type, INDIRECT_NAME) || !ft_strcmp\
    (pointer->type, INDIRECT_LABEL_NAME))
		(*struct_content)->memory_code_size += TWO_BYTE;
	else if (!ft_strcmp(pointer->type, DIRECT_NAME) || !ft_strcmp\
    (pointer->type, DIRECT_LABEL_NAME))

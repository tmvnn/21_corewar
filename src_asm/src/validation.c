...
{
	if (parse(content->line, PATTERN_HELP_VALIDATION_FIRST_CASE))
		content->line = rebase_str_first_case(content->line);
	if (parse(content->line, PATTERN_HELP_VALIDATION_SECOND_CASE))
		content->line = rebase_str_second_case(content->line);
	if (parse(content->line, PATTERN_HELP_VALIDATION_THIRD_CASE))
		content->line = rebase_str_third_case(content->line);
	if (parse(content->line, PATTERN_HELP_VALIDATION_FOURTH_CASE))
...
char	*check_valid(t_asm_content *content, int fd)
{
	...
	PATTERN_NAME_OR_COMMENT_CHAMPS_FIRST_STAGE))
